...
export default async function OrganizationConfigurePage({
  searchParams,
}: {
  searchParams: Promise<{ tab?: string }>;
}) {
  const params = await searchParams;
  const tab = TABS.some((t) => t.id === params.tab) ? params.tab! : "regional";
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold">Advanced configuration</h1>
        <p className="text-sm text-muted-foreground">
          Regional, hierarchy, EHS, notification, and security settings for this tenant.
        </p>
      ...
      <SettingsNav current="/app/settings/organization/configure" />
      <div className="flex flex-wrap gap-2">
        {TABS.map((t) => (
          <Link
            key={t.id}
            href={`/app/settings/organization/configure?tab=${t.id}`}
            className={
              tab === t.id
      ...
      >
        ...
        {tab === "hierarchy" ? (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Enable only the levels your organization uses.
            </p>
            {(
              [
