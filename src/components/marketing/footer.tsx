  ...
  return (
    <footer className="border-t border-white/10 bg-[var(--mkt-hero)] text-slate-300">
      <Container className="py-16 md:py-20">
              ...
              <ul className="mt-4 space-y-2.5">
                {column.links.map((link) => (
                  <li key={link.href + link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-slate-300 transition-colors hover:text-white motion-reduce:transition-none"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-16 flex flex-col gap-2 border-t border-white/10 pt-6 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
