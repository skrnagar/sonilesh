    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Badge variant="success">Live view</Badge>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "Open incidents", value: "12", tone: "text-primary" },
            { label: "Overdue CAPA", value: "4", tone: "text-[#b45309]" },
            { label: "Permits active", value: "27", tone: "text-[#0f766e]" },
          ].map((m) => (
            <div key={m.label} className="rounded-md border border-[#d7dee7] bg-white px-2.5 py-2">
              <p className="text-[10px] uppercase tracking-[0.1em] text-slate-500">
                {m.label}
              </p>
              <p className={cn("mt-1 text-lg font-semibold tabular-nums", m.tone)}>
                {m.value}
              </p>
            </div>
          ))}
